...
matrix factorization을 이용한 방법이다. 
n=# of nodes, d=dim of attribute vector, N=# of required cluster일 때, \\(S_{n{\times}n}\\)은 adjacency matrix, \\(A_{n{\times}d}\\)는 node attribute matrix, \\(U_{n{\times}N}\\)은 각 node-cluster 연관성 보여주는 cluster membership matrix, \\(V_{d{\times}N}\\)는 각 attribute와 cluster 연관 알려주는 cluster membership matrix이다.
그런 조건들이 주어질 때 \\(S\\)와 \\(UU_t\\)의 차이를 가장 작게하는 \\(U\\)와 \\(U\\)와 \\(AV\\)의 차이를 가장 작게하는 V를 구하는 문제로 전환할 
