...
- n = # of nodes, 
- d = dim of attribute vector
- N = # of required cluster 일 때 


- \\(S_{n{\times}n}\\)은 adjacency matrix, 
- \\(A_{n{\times}d}\\)는 node attribute matrix,
- \\(U_{n{\times}N}\\)은 각 node-cluster 연관성 보여주는 cluster membership matrix, 
- \\(V_{d{\times}N}\\)는 각 attribute-cluster 연관성 보여주는 cluster membership matrix로 정의한다면,
